...
price_regex = re.compile(r'\$?\d{0,}\,\d{0,}')

#Make this process a while loop 
#split by newlines
listing = test['name'].encode('utf-8').strip().split("\n")
#assign to new variables standing for the columns
address = listing[0]
city_state_zip = listing[1]
bed_bath_price = listing[2].split(", ")
bedsize = int(re.findall(size_regex, bed_bath_price[0])[0])
bathsize = int(re.findall(size_regex, bed_bath_price[1])[0])
